...
interface ArchiveTradeCategoryProps {
    open: boolean;
    onClose: () => void;
    onWorkUpdated?: () => void;
}
...
const ArchiveTradeCategory: React.FC<ArchiveTradeCategoryProps> = ({
                                                                           open,
                                                                           onClose,
                                                                           onWorkUpdated,
                                                                       }) => {
    ...
    const fetchArchiveAddress = async () => {
        try {
            ...
            const res = await api.get(
                `trade/archive-category-list?company_id=${user.company_id}`
            );
            if (res.data?.info) {
    ...
    const handleConfirmAction = async () => {
        ...
        try {
            ...
            if (selectedItem.action === 'restore') {
                let response = await api.post('trade/unarchive-trade-category', payload);
                if (response.data.IsSuccess == true) {
                    toast.success(response.data.message);
                    onWorkUpdated?.();
                }
            } else if (selectedItem.action === 'delete') {
                let response = await api.post(`trade/delete-trade-category`, payload);
                if (response.data.IsSuccess == true) {
                    toast.success(response.data.message);
                    onWorkUpdated?.();
                }
...
export default ArchiveTradeCategory;
