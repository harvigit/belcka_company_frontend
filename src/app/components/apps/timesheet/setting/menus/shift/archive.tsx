...
interface ArchiveShiftsProps {
  open: boolean;
  onClose: () => void;
  onWorkUpdated?: () => void;
}

const ArchiveShifts: React.FC<ArchiveShiftsProps> = ({
  open,
  onClose,
  onWorkUpdated,
}) => {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      if (selectedItem.action === "restore") {
        ...
        let response: AxiosResponse<any> = await api.delete(
          `shift/delete?shift_id=${selectedItem.id}`
        );
        if (response.data.IsSuccess == true) {
          toast.success(response.data.message);
        }
