...
import CreateProjectRole from "./create";
import EditProjectRole from "./edit";

const ProjectRolesList = () => {
  const [data, setData] = useState<any[]>([]);
  const [fetchRole, setFetchRole] = useState<boolean>(true);
  ...
  const { columnVisibility, onColumnVisibilityChange } =
    usePersistentColumnVisibility({
      storageKey: `cv_${company?.company_id}_${company?.id}_project_roles`,
      enabled: !!company?.id,
    });
  ...
  const fetchProjectRoles = async () => {
    setFetchRole(true);
    try {
      let url = `project-roles/get?company_id=${company?.company_id}&page=${pagination.pageIndex + 1}&limit=${pagination.pageSize}`;
      if (searchTerm) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const result = await api.post("project-roles/create", {
        ...formData,
      });
  ...
  const editProjectRole = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsSaving(true);
    try {
      const result = await api.post("project-roles/update", {
        ...formData,
      });
      if (result.data.IsSuccess == true) {
        toast.success(result.data.message);
  ...
  const { table, pagination, setPageCount, totalRows, setTotalRows } =
    useServerTable({
      data: data,
      columns,
      fetchData: fetchProjectRoles,
      ...
      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
        ...
        <DialogContent>
          <Typography color="textSecondary">
            Are you sure you want to delete {rolesToDelete.length} role
            {rolesToDelete.length > 1 ? "s" : ""}?
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
