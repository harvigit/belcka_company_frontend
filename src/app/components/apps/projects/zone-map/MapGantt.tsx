                ...
                return (
                  <Box
                    ...
                    sx={{
                      ...
                      "&:hover": {
                        backgroundColor: selected ? "#1565c0" : "#f0f4ff",
                      },
...
export default function MapGantt({
  ...
  const fetchUserLocationsWithFilters = async (
    activeFilters: typeof filters,
  ) => {
  ...
  }, [open, activeProjectId, activeTab, apiDateTime, projectId]);
  ...
  const totalZonePages = Math.max(
    1,
    Math.ceil(filterData.length / zonePageSize),
  );
  ...
  const groupedUsers = useMemo(
    () => groupByTeam(filteredUserLocations),
    [filteredUserLocations],
  );

  const usersWithLocation = useMemo(
    () =>
      spreadOverlappingUsers(
              ...
              SelectProps={{
                ...
                renderValue: (selected) =>
                  (selected as number[])
                    .map(
                      (id) =>
                        resources.trades.find((t: any) => t.id === id)?.name ??
                        id,
                    )
                    .join(", "),
              }}
              ...
              SelectProps={{
                ...
                renderValue: (selected) =>
                  (selected as number[])
                    .map(
                      (id) =>
                        resources.teams.find((t: any) => t.id === id)?.name ??
                        id,
                    )
                    .join(", "),
              }}
      ...
      >
              ...
              >
                ...
                >
                  ...
                  <IconButton
                    ...
                    onClick={() => setZonePage((p) => p + 1)}
                    disabled={
                      (zonePage + 1) * zonePageSize >= filterData.length
                    }
                  >
                    <IconChevronRight />
                  </IconButton>
        ...
        >
          <Box
            sx={{
              display:
                !addZoneOpen && selected?.mode !== "edit" ? "flex" : "none",
              flex: 1,
              flexDirection: "column",
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {isExpanded ? (
                      <IconChevronDown size={17} color="#888" />
                    ) : (
        ...
        {hovered && (
          ...
          >
            ...
            <Box display="inline-flex" alignItems="center" gap={0.5} mt={0.5}>
              ...
              <Typography
                variant="caption"
                sx={{ color: dotColor, fontWeight: 600 }}
              >
        ...
        {zones.map((zone) => {
          ...
          if (zone.type === "polygon") {
            if (!Array.isArray(zone.coordinates) || zone.coordinates.length < 3)
              return null;
            const path = zone.coordinates.map((p: any) => ({
              lat: Number(p.lat),
              lng: Number(p.lng),
            }));
            const centroid = {
              lat:
                path.reduce((s: number, p: any) => s + p.lat, 0) / path.length,
              lng:
                path.reduce((s: number, p: any) => s + p.lng, 0) / path.length,
            };
            return (
              <React.Fragment key={zone.id}>
                <OverlayView
