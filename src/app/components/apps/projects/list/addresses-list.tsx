...
const AddressesList = ({ projectId, searchTerm, filters }: AddressesListProps) => {
    ...
    const currentFilteredData = useMemo(() => {
        let filtered = data.filter((item) => {
            ...
            const matchesSearch = item.name.toLowerCase().includes(search) || item.progress.toLowerCase().includes(search);
        ...
        columnHelper.accessor('name', {
            ...
            header: () => (
                <Stack direction="row" alignItems="center" spacing={4}>
                    ...
                    />
                    <Typography variant="subtitle2" fontWeight="inherit">Address</Typography>
                </Stack>
            ),
            enableSorting: true,
            cell: ({ row }) => {
