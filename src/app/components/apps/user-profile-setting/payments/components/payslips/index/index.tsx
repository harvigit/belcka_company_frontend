...
const PayslipsList: React.FC<Props> = ({userId, isShow, disableDateFilter}) => {
    ...
    // Fetch data
    const fetchPayslips = async (start: Date | null, end: Date | null): Promise<void> => {
        setFetchPayslip(true);
        try {
            const startParam = start ? format(start, 'dd/MM/yyyy') : '';
            const endParam = end ? format(end, 'dd/MM/yyyy') : '';
            const res = await api.get(
                `payslips/get?company_id=${user.company_id}&user_id=${userId}&start_date=${startParam}&end_date=${endParam}`,
            );
            if (res.data) {
