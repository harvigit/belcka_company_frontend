...
import {
    Avatar, Box, Divider, IconButton, MenuItem, Stack, Table, TableBody,
...
import {
    ...
    IconSettings, IconPencil, IconTrash,
...
const INITIAL_FORM: FormState = { hazardId: '', description: '' };
        ...
        if (af.formatKey === 'audio' && af.previewUrl) {
            return (
                <Box sx={{
                    ...
                    <Box component="audio" src={af.previewUrl} controls
                         sx={{ width: '90%', maxWidth: 340 }}
                    />
                </Box>
            );
        }
        if (af.formatKey === 'pdf') {
            return (
                <Box sx={{
                    ...
                    <Typography variant="caption" color={meta.color} fontWeight={600}>
                        Click to open PDF
                    </Typography>
                </Box>
            );
            ...
            <Box sx={{ p: 3 }}>
                <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                    <Box flex={1} pr={2} minWidth={0}>
                        <Typography variant="body1" fontWeight={600} noWrap title={af.name}>
                            {af.name}
                        </Typography>
...
const downloadAttachment = async (af: AttachmentFile, onError: (msg: string) => void) => {
    ...
    // PDFs — direct link (avoids CORS blob issues)
    if (af.formatKey === 'pdf') {
        triggerLink(url, af.name || 'file.pdf', true);
        return;
    }

    // Other media — try blob download first, fallback to direct link
    try {
...
const NearMissReporting = ({ companyId }: Props) => {
    ...
    const [hazardOptions, setHazardOptions] = useState<any[]>([]);
    ...
    const fetchResources = useCallback(async () => {
        ...
        try {
            ...
            if (res.data.IsSuccess) setHazardOptions(res.data.hazards || []);
        } catch {}
    }, [companyId]);
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const res = await api.get('/near-miss-reports/get', { params: { company_id: companyId } });
            if (res.data.IsSuccess) {
                setData((res.data.info || []).map((item: any) => ({
                    id: item.id,
                    added_by: item.added_by_user_name || '-',
                    user_thumb_image: item.added_by_user_thumb_image || item.added_by_user_image || '',
                    description: item.description || '',
                    hazard_id: item.hazard_id,
                    hazard_type: item.hazard_name || '-',
            ...
            showError('Failed to fetch near miss reports.');
        } finally {
    ...
    const handleEdit = useCallback((row: NearMissRow) => {
        resetAttachments();
        setIsEditing(true);
        setEditingId(row.id);
        setForm({
            hazardId: String(row.hazard_id || ''),
    ...
    const handleSubmit = useCallback(async () => {
        if (!form.hazardId) { showError('Please select a hazard type.'); return; }
        setSubmitting(true);
        try {
            ...
            const res = await api.post('/near-miss-reports/store', formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });

            if (res.data.IsSuccess) {
                showSuccess(isEditing ? 'Near miss report updated successfully.' : 'Near miss report submitted successfully.');
                resetAndClose();
                fetchData();
            } else {
    ...
    const handleViewAttachments = useCallback((row: NearMissRow) => {
    ...
    const confirmDelete = useCallback(async () => {
        ...
        try {
            const res = await api.delete(`/near-miss-reports/delete/${deletingId}`);
            if (res.data.IsSuccess) {
                showSuccess('Near miss report deleted successfully.');
                fetchData();
            } else {
                showError(res.data.message || 'Failed to delete.');
            }
        } catch {
            showError('Failed to delete near miss report.');
        } finally {
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            header: 'Added By',
            cell: ({ row }: any) => {
                const r = row.original as NearMissRow;
                return (
    ...
    return (
        <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap', px: 2, py: 1.5 }}>
                ...
                <Settings settingOpen={settingOpen} companyId={companyId} onClose={() => { setSettingOpen(false); fetchData(); }} />
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                    <Box sx={{
                        ...
                        <Typography fontWeight={700} fontSize="1.05rem">
                            {isEditing ? 'Edit Near Miss Report' : 'Add Near Miss Report'}
                        </Typography>
                        ...
                        <Stack spacing={3}>
                            <Box>
                                ...
                                <Autocomplete
                                    ...
                                    value={hazardOptions.find(o => String(o.id) === form.hazardId) || null}
                                    onChange={(_, v) => setForm(f => ({ ...f, hazardId: v?.id ? String(v.id) : '' }))}
                                    getOptionLabel={o => o.title || ''}
                                    isOptionEqualToValue={(o, v) => String(o.id) === String(v?.id)}
                                    renderInput={params => <CustomTextField {...params} placeholder="Select Hazard Type" fullWidth />}
                                />
                            </Box>
