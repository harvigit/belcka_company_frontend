...
const RemoveUsersList = () => {
    ...
    const fetchUsers = async () => {
        setFetchUser(true);
        try {
            const res: AxiosResponse<any> = await api.get('user/get-remove-users');
            if (res.data) {
    ...
    const filteredData = useMemo(() => {
        return data.filter((item) => {
            ...
            return (
                ...
                item.trade_name?.toLowerCase().includes(s) ||
                item.supervisor_name?.toLowerCase().includes(s) ||
    ...
    const getColumnVisibilityKey = (userId?: number | string) =>
        userId ? `columnVisibility_removeUsers_${userId}` : 'columnVisibility_removeUsers';
    ...
    const columns = [
        ...
        columnHelper.accessor((row) => row.team_name, {
            id: 'teamName',
            header: () => <Typography variant="subtitle2" noWrap>Team Name</Typography>,
            cell: (info) => (
                <Typography className="f-14" color="textPrimary" sx={{ width: 100, ml: 2 }}>
                    {info.getValue() ?? '-'}
                </Typography>
            ),
        }),
        columnHelper.accessor((row) => row.email, {
            id: 'email',
            header: () => <Typography variant="subtitle2" noWrap>Email</Typography>,
            cell: (info) => (
        ...
        columnHelper.accessor((row) => row.phone, {
            id: 'phone',
            header: () => <Typography variant="subtitle2" noWrap>Phone</Typography>,
            cell: (info) => {
                const u = info.row.original;
                return (
                    <Typography className="f-14" color="textPrimary">
                        {u.extension ?? '0'}{info.getValue() ?? '-'}
                    </Typography>
                );
            },
        }),
        columnHelper.accessor((row) => row.joining_date, {
            id: 'joiningDate',
            header: () => <Typography variant="subtitle2" noWrap>Joining on</Typography>,
            cell: (info) => {
        ...
        columnHelper.accessor((row) => row.remove_on, {
            id: 'removeDate',
            header: () => <Typography variant="subtitle2" noWrap>Remove on</Typography>,
            cell: (info) => {
                const row = info.row.original;
                return (
                    <Typography className="f-14" color="textPrimary">
    ...
    useEffect(() => {
        const eligibleColumns = table.getAllLeafColumns().filter((col) => col.id !== 'conflicts');
    ...
    const handleSelectAllChange = (e: any) => {
        ...
        Cookies.set(columnVisibilityKey, JSON.stringify({ ...newVisibility, selectAll: checked }), { expires: 365 });
    ...
    const handleColumnVisibilityChange = (colId: string, value: boolean) => {
        ...
        Cookies.set(columnVisibilityKey, JSON.stringify(updatedVisibility), { expires: 365 });
    ...
    const visibleColumns = table.getAllLeafColumns().filter((col) => col.id !== 'conflicts' && col.getIsVisible());
    const columnData = visibleColumns.length ? visibleColumns : columns;
    const simpleColumns = columnData.map((column: any) => ({ name: column.id ?? 'Unnamed Column', width: 'auto' }));
