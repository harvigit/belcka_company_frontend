...
const TablePagination = () => {
  ...
  const fetchExpenseCategories = async () => {
    ...
    try {
      const res = await api.get(
        `company-expense-categories/get?company_id=${id.company_id}`
      );
      if (res.data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await api.post("company-expense-categories/create", payload);
  ...
  const editExpenseCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await api.put("company-expense-categories/update", payload);
      if (result.data.IsSuccess == true) {
        toast.success(result.data.message);
        setFormData({
          name: "",
          ...
          <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
            ...
            <DialogContent>
              <Typography color="textSecondary">
                Are you sure you want to archive {usersToDelete.length} expense category
                {usersToDelete.length > 1 ? "s" : ""} from the expense categories?
              </Typography>
            </DialogContent>
            <DialogActions>
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    const response = await api.post(
                      "company-expense-categories/archive",
                      payload
                    );
