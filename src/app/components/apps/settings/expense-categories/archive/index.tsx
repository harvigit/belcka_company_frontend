...
interface ArchiveExpenseCategoryProps {
    open: boolean;
    onClose: () => void;
    onWorkUpdated?: () => void;
}
...
const ArchiveExpenseCategory: React.FC<ArchiveExpenseCategoryProps> = ({
                                                                           open,
                                                                           onClose,
                                                                           onWorkUpdated,
                                                                       }) => {
    ...
    const handleConfirmAction = async () => {
        ...
        try {
            ...
            if (selectedItem.action === 'restore') {
                let response = await api.post('expense-categories/unarchive', payload);
                if (response.data.IsSuccess == true) {
                    toast.success(response.data.message);
                    onWorkUpdated?.();
                }
            } else if (selectedItem.action === 'delete') {
                let response = await api.post(`/expense-categories/delete`, payload);
                if (response.data.IsSuccess == true) {
                    toast.success(response.data.message);
                    onWorkUpdated?.();
                }
...
export default ArchiveExpenseCategory;
