...
const GeneralSetting: React.FC<GeneralSettingProps> = ({ onSaveSuccess }) => {
    ...
    const handleRemoveUserLimit = (index: number) => {
        const removedEntry = setting.user_limits[index];
        if (removedEntry?.user_id !== null && removedEntry?.user_id !== undefined) {
            setRemovedUserIds((ids) => [...new Set([...ids, Number(removedEntry.user_id)])]);
        }
        setSetting((prev) => {
            return {
                ...prev,
    ...
    const handleUserLimitChange = (index: number, field: 'user_id' | 'limit', value: any) => {
        if (field === 'user_id') {
            const previousUserId = setting.user_limits[index]?.user_id;
            if (previousUserId !== null && previousUserId !== undefined && previousUserId !== Number(value)) {
                setRemovedUserIds((ids) => [...new Set([...ids, Number(previousUserId)])]);
            }
            if (value !== null) {
                setRemovedUserIds((ids) => ids.filter((id) => id !== Number(value)));
            }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const res = await api.post(`setting/leaves-general-settings`, payload);
            if (res.data?.IsSuccess) {
                                ...
                                {setting.user_limits.length === 0 ? (
                                    ...
                                    <Stack spacing={1.5}>
                                        {setting.user_limits.map((entry, index) => {
                                            const selectedUser = users.find((u) => u.id === entry.user_id) || null;
