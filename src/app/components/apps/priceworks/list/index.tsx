...
const PriceworkList = ({
    ...
    const priceworkPreferencesKey = useMemo(
        () =>
            user?.id && user?.company_id
                ? `pricework-list-filters_${user.id}_${user.company_id}`
                : '',
        [user?.id, user?.company_id],
    );
    ...
    const startEditPriceworkAmountCell = (
        ...
        const currentValue = field === 'amount_per_unit' ? Number(row.amount_per_unit || 0) : Number(row.work_complete || 0);
    ...
    const savePriceworkAmountCell = async (
        ...
        const nextAmountPerUnit = field === 'amount_per_unit' ? nextValue : currentAmountPerUnit;
        const nextWorkComplete = field === 'work_complete' ? nextValue : currentWorkComplete;

        if (Math.abs(nextAmountPerUnit - currentAmountPerUnit) < 0.00001 && Math.abs(nextWorkComplete - currentWorkComplete) < 0.00001) {
            cancelEditPriceworkAmountCell();
            return;
        }
        ...
        try {
            ...
            toast.error(
                error?.response?.data?.message || 'Failed to update pricework amount',
            );
        } finally {
    ...
    const selectedTotal = useMemo(() => {
        if (isSelectAll) {
            return data.reduce(
                (sum, item) => sum + Number(item.pricework_amount || 0),
                0,
            );
        }
        return data
    ...
    const handleApprovePriceworks = async (
        ...
        try {
            ...
            toast.error(
                error?.response?.data?.message || 'Failed to approve pricework',
            );
        }
    };
    ...
    const handleRejectPriceworks = async (
        ...
        try {
            ...
            toast.error(
                error?.response?.data?.message || 'Failed to reject pricework',
            );
        } finally {
    ...
    const handleSendToBookkeeper = async () => {
        ...
        try {
            ...
            toast.error(
                ...
                'Failed to send priceworks to bookkeeper',
            );
        }
    };
    ...
    const handleUnapproveSelected = async () => {
        ...
        try {
            ...
            toast.error(
                error?.response?.data?.message || 'Failed to unapprove priceworks',
            );
        } finally {
    ...
    const handleDeletePriceworks = async () => {
        ...
        try {
            ...
            toast.error(
                error?.response?.data?.message ||
                'An error occurred while deleting priceworks',
            );
        } finally {
            setIsDeleting(false);
        }
        ...
        () => [
            {
                    ...
                    <Stack direction="row" alignItems="center">
                        <CustomCheckbox
                            ...
                            checked={
                                ...
                                (data.length > 0 &&
                                    data.every((row) =>
                                        selectedRowIds.has(getPriceworkRowKey(row)),
                                    ))
                            }
                            indeterminate={
                                !isSelectAll &&
                ...
                cell: ({row}: { row: { original: PriceworkRow } }) => {
                    ...
                    const isChecked =
                        isSelectAll || selectedRowIds.has(getPriceworkRowKey(item));
                    return (
                        <Stack direction="row" alignItems="center">
                            <CustomCheckbox
                                checked={isChecked}
                                    ...
                                    InputProps={{
                                        startAdornment: (
                                            <InputAdornment position="start">
                                                <Typography className="f-14">{currency}</Typography>
                                            </InputAdornment>
                                        ),
                                    }}
                                />
    ...
    return (
        ...
        >
                            ...
                            {loading ? (
                                ...
                                table.getRowModel().rows.map((row) => (
                                    <TableRow
                                        ...
                                        selected={
                                            isSelectAll ||
                                            selectedRowIds.has(getPriceworkRowKey(row.original))
                                        }
                                    >
            ...
            >
                ...
                <DialogContent>
                    <Typography>
                        Are you sure you want to delete {selectedCount} selected pricework
