...
const PriceworkList = () => {
  ...
  const [attachmentsPricework, setAttachmentsPricework] =
    useState<PriceworkRow | null>(null);
  ...
  const [rejectTimesheetLightIds, setRejectTimesheetLightIds] = useState<
    number[]
  >([]);
  ...
  const selectedCurrency =
    data.find((item) => selectedRowIds.has(getPriceworkRowKey(item)))
      ?.currency ||
    data[0]?.currency ||
    "£";
  ...
  const getActionPriceworkIds = () =>
    getSelectedRows()
      .filter(isStandalonePriceworkRow)
      .map((item) => item.id);
