...
const ExpenseList = ({projectId}: { projectId?: number } = {}) => {
    ...
    const [savingAmountIds, setSavingAmountIds] = useState<Set<number>>(
        new Set(),
    );
    ...
    const saveExpenseAmount = async (row: ExpenseRow, rawValue: string) => {
        ...
        try {
            ...
            toast.error(
                error?.response?.data?.message || 'Failed to update expense amount',
            );
        } finally {
    ...
    const fetchExpenses = async () => {
        ...
        try {
            ...
            const activeSorting = sorting[0] ?? getDefaultExpenseSorting()[0];
            url += `&sort_by=${activeSorting.id}&sort_order=${activeSorting.desc ? 'desc' : 'asc'}`;
    ...
    const selectedCount = isSelectAll ? listItems.length : selectedRowIds.size;
    ...
    const getApprovedActionExpenseIds = () => {
        ...
        return listItems
            .filter(
                (item) => selectedIdSet.has(item.id) && item.status === 'approved',
            )
            .map((item) => item.id);
    };
    ...
    const handleDeleteExpenses = async () => {
        ...
        try {
            ...
            toast.error(
                ...
                'An error occurred while deleting expenses',
            );
        } finally {
            setIsDeleting(false);
        }
    ...
    const handleSendToBookkeeper = async () => {
        ...
        try {
            ...
            toast.error(
                error?.response?.data?.message ||
                'Failed to send expenses to bookkeeper',
            );
        }
    };
    ...
    return (
        ...
        >
            ...
            >
                ...
                <DialogContent>
                    <Typography>
                        Are you sure you want to delete {selectedCount} selected expense
